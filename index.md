...
杨青松是憨憨时间管理应用程序的运营者，本政策仅适用于杨青松的憨憨时间管理产品或服务。如果您决定使用我们的服务，此页面用于向您说明在您使用我们的产品或服务时如何个人信息收集、使用和披露政策。              
...
第三方SDK收集您的信息有：个人常用设备信息（包括硬件序列号、设备**MAC地址**、软件安装列表、唯一设备识别码IMEl/android ID/IDFA/OPENL JDID/GUID/SIM卡IMSI信息等）、硬件型号、操作系统版本号、网络设备硬件地址（MAC）、IP地址、软件版本号、安装列表、网络接入方式及类型、操作日志等信息、手机号权限、陀螺仪传感器/加速度传感器。      
      
**操作日志等信息需要每30秒上传一次且数据为匿名上传**，目的是了解用于群体使用习惯以便优化产品功能。    
**陀螺仪传感器/加速度传感器**，目的是在加载开屏广告后，用户通过摇动手机进入广告的着落页。
...
**方式：** 基于用户授权获取          
**范围：** SDK/API/JS代码版本、浏览器、终端设备操作系统版本、互联网服务供应商、IP地址、应用分发渠道、独立设备标识符、iOS广告标识符（IDFA)、安卓广告主标识符、网卡（MAC）地址、国际移动设备识别码（IMEI）、设备型号、浏览的页面、访问的时间和长度、下载、点击、软件安装列表              
**隐私权政策链接地址：** [adview隐私政策](https://policies.google.cn/privacy) 
...
**权限使用说明二：** 我们的产品集成了adview SDK，adview SDK需要收集您的设备Mac地址、软件安装列表、唯一设备识别码（IMEI/android ID/IDFA/OPENUDID/GUID、SIM 卡 IMSI 信息）用于广告服务        
...
**权限使用说明：** 日程及计时记录功能以及备份数据的读取需要存储权限          
**权限授权方式：** 应用启动页面         
**是否可关闭：** 是              
                   
* **3）权限名称：** 相机权限（android.permission.CAMERA）           
**权限使用说明：** 读取存储在手机里的背景图片               
**权限授权方式：** 应用启动页面            
**是否可关闭：** 是             
...
**权限使用说明:**  用于同步日程到系统日历，以及获取日程创建的时间 、任务计时的时间点              
**权限授权方式：** 应用启动页面                
**是否可关闭：** 是          
       
**3. 在您使用服务过程中需要过安卓系统发送广播唤醒APP自启动或关联启动**   
为了方便用户在手机桌面能够快速查看日程，我们提供了桌面小部件功能。小部件的日程信息需要与软件中的日程同步，所以需要过安卓系统发送广播唤醒APP自启动或关联启动。
